...
定義しなくても動きます。**その配色で不都合が出るテーマだけが定義します。**

| 変数 | 既定の扱い | 定義するのはどんなとき |
|---|---|---|
| `--on-primary-color` | `--surface` を使う | 基調色を敷いた面に載る文字の色です。**基調色が暗いテーマは前面が明るく、基調色が明るいテーマは前面が暗い**ので、たいていは `--surface` で読めます。そうならない配色のときだけ定義します |
| `--color-scheme` | OSの設定に従う（`light dark`） | ブラウザが描く部品（日付ピッカー・ファイル選択・スクロールバー）の明暗です。**明暗の片方に固定しているテーマ**が `dark`（または `light`）を入れます。両方を持つテーマは定義しません |

どちらも[編集画面](#編集画面もトークンを読む)が読みます。
...
出店者エディタ（`core/editor/`）は、来場者が見る画面ではありません。
テーマ仕様のクラス名とも無関係で、**スロットを置く必要も、CSSを書く必要もありません。**

それでも、**配色・書体・角丸・影だけはこのトークンから引きます**（[決定19](decisions.md)）。
運営と出店者にとっては、サイトと編集画面が同じ道具に見えるほうが自然だからです。

```html
<!-- 配置後の editor/index.html。テーマのトークンだけを読む -->
<link rel="stylesheet" href="../css/tokens.css">
<link rel="stylesheet" href="editor.css">
```

読むのは `tokens.css` だけです。**`layout.css` も `components.css` も読みません。**
変数の定義しか入っていないので、編集画面のレイアウトには影響しません。

| テーマの置き方 | 編集画面の見た目 |
|---|---|
| `css/tokens.css` に置いた | **そのテーマの配色になる** |
| 置いていない（テーマを当てないサイト） | **既定テーマと同じ配色になる**（`editor.css` が同じ値を持っています） |

テーマ側ですることは何もありません。
`tokens.css` に上の変数が揃っていれば、それだけで編集画面もついてきます。

## 2. コアが探すスロット

**スロット**とは、テーマがHTMLに置く空の要素のことです。コアはそれを探して中身を作ります
（[3層モデルとスロット](concepts.md#スロット--halleとauventの接点)）。

**テーマのHTMLに用意してください。** 無いと該当機能が動きません。

### 出店者グリッド

カテゴリごとにスロットを1つ置きます。属性値は `shops.json` のカテゴリIDです。
...
置き忘れると、**その区分の出店者はどこにも出ません。エラーも出ません。**
`tools/validate.py` が `shops.json` と突き合わせて指摘します。
...
ポップアップの中身のスロットは次の名前です。**要らないものはスロットごと消して構いません。**
コアはスロットのあるものだけを埋めます。
...
**畳まないなら `.menu-toggle-button` のスロットごと消して構いません**（スロットの無い機能は動かないだけです）。

ポップアップの `*-overlay` / `*-close-button` / `*-image` / `*-name` / `*-price` /
`*-description` などは、**テーマが用意した空のスロットにコアが値を流し込む**構造です。
スロットの見た目はテーマの自由ですが、クラス名と入れ子の関係は維持してください。
