...
| | 必要なもの |
|---|---|
| サーバー | PHP 8.0以上が動く一般的なレンタルサーバー。**データベースは要りません** |
...
| `site.social` が無い | SNSのリンクが**器ごと**隠れます |
...
区分（`categories`）を増やすときは、**テーマの `index.html` にも器を1行足します。**
...
まとめて報告します。**出店者がまだ1店も書き込んでいない段階でも通ります。**
...
| 2 | お知らせのセクションが**丸ごと消えている** | `news.json` が `[]` なら正常（0件はセクションごと隠します） |
| 3 | 出店者の区分が出る（中身は空でよい） | `data/shops.json` が読めている |
| 4 | 文言が日本語で出る（`common.loading` のようなキー名が出ない） | `i18n/` の配置 |
| 5 | 運営向けUIが出る | `https://example.com/editor/?<管理キー>` |
| 6 | 店舗IDを入れて読み込める | ロスターに登録済みのID |
| 7 | 紹介文と商品を入れて**保存できる** | `data/shop-data/<店舗ID>/data.json` ができる |
| 8 | 保存した内容がサイトに出る | トップを再読み込み |
| 9 | お知らせを1件追加できる | `https://example.com/editor/news/?<管理キー>` |
| 10 | Webhookに通知が届く | 設定した場合のみ |
...
画面もサーバー側も動きます。テーマに器を1つ置き、項目定義のJSONを用意するだけです。
...
**器はこの1行だけです。** 入力欄・確認モーダル・送信はコアが受け持ちます。
同梱の2テーマにはすでに入っています。

`data-marche-form-section` で包むと、**定義が読めないときに見出しごと隠れます。**
包まないと器だけが隠れ、「お問い合わせ」の見出しと余白が残ります。
...
| ポップアップが開いたまま見える | テーマのCSSに「仕掛け」が足りない | [テーマ仕様](theme-contract.md#テーマが用意する仕掛け)の6つを確認する |
...
| 開催年を変える | `marche.config.json` の `event.year` と `days` を書き換える |
