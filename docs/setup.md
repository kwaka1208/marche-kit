...
[リリース](https://github.com/kwaka1208/marche-kit/releases)のzipでも、gitでも取れます。
...
やっているのは2つだけなので、手作業でも同じ結果になります。

1. `core/php/secrets.example.php` を `<配置用>/data/secrets.php` として複製し、値を書く
   （**`secrets.example.php` のほうは配置しません**）
2. `<配置用>/editor/editor.js` と `<配置用>/editor/news/news-editor.js` の
   `__ADMIN_KEY__` `__WEBHOOK_URL__` を実際の値に置き換える

パーミッションは手順8のとおり `secrets.php` を `600` にしてください。

**サーバー側で環境変数を設定できるなら、`secrets.php` を置かなくても構いません。**
`MARCHE_ADMIN_KEY` などで与えれば、そちらが優先されます。
