...
| `news/` | 運営 | お知らせの追加・修正 |
| `editor.css` | — | 両方の見た目。**配色はテーマから引きます**（下記） |
...
配色・書体・角丸・影は、サイトに置いたテーマから引きます（[決定19](../../docs/decisions.md)）。
両ページが `tokens.css` を読んでいるのはこのためです。

```html
<link rel="stylesheet" href="../css/tokens.css">   <!-- テーマの変数 -->
<link rel="stylesheet" href="editor.css">          <!-- 編集画面の見た目 -->
```

| テーマの置き方 | 編集画面 |
|---|---|
| `<公開ディレクトリ>/css/tokens.css` に置いた | **そのテーマの配色になる**（`night-market` なら暗い配色） |
| 置いていない | **既定テーマ（`themes/default`）と同じ配色になる** |

**読めなくてもかまいません。** `editor.css` の `--editor-*` が既定テーマと同じ値を
フォールバックとして持っているので、404になっても見た目は崩れません。

編集画面だけを別の配色にしたいときは、`editor.css` の `--editor-*` を上書きします。
サイトごと色を変えたいときは、テーマの `tokens.css` を直してください。

**完売・販売終了・送信結果の色だけは、テーマから引きません。** 運用の状態を示す色で、
イベントの配色ではないためです。そのときの面の色に混ぜて作るので、
明るいテーマでも暗いテーマでも同じ濃さに見えます。
